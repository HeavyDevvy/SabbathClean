...
- **House Cleaning Enhancements (Nov 13, 2025)**: Service-specific booking enhancements exclusively for House Cleaning: (1) **Enhanced Provider Cards**: Display provider bio (line-clamp-2), profile images (16x16 rounded), jobs completed + years experience, qualifications as badges, and availability vs specializations. Provider data kept generic to avoid contamination across services. (2) **Tipping Functionality**: Provider tipping UI at step 4 with preset buttons (R0, R20, R50, R100) and custom input (0-1000 range, 2 decimal precision, NaN/empty string validation). Tips stored separately in cart payload and serviceDetails JSON, excluded from subtotal, and exempt from 15% platform commission. Tips displayed with green success color throughout booking flow for visual distinction. (3) **3-Button CTA Layout**: Modified footer buttons showing "Back", "Add to cart & Select Another", and "Add & Go to Cart" for streamlined booking flow. (4) **Estimated Hours Logic**: 6-hour minimum baseline + 0.5 hours per add-on (implemented in config/estimates.ts). (5) **Database Schema**: Added tip_amount column (DECIMAL(10,2) DEFAULT 0) to both cart_items and order_items tables. Checkout flow transfers tips from cart to order, includes tips in total calculation (subtotal + tips + platformFee), and persists through complete booking lifecycle (cart → checkout → order → receipt). (6) **Tip Display**: Tips shown in cart-checkout page (individual items + payment summary) and order-confirmation page (individual items + payment summary) with "Provider Tips" label and green text-success styling. Platform fee calculated on subtotal only, excluding tips.
- **Customizable User Profiles (Nov 15, 2025)**: Comprehensive profile customization system with three key features: (1) **Preferred Services**: Multi-select interface allowing users to mark their frequently used services from 8 categories (house-cleaning, garden-care, plumbing, electrical, chef-catering, event-staff, moving, au-pair) for personalized recommendations. Visual selection with icons, togglable badges, and real-time updates. (2) **Preferred Providers**: Save favorite service providers for quick rebooking with ability to add/remove providers from booking history. Displays provider cards with profile images and quick-access controls. (3) **Database Schema**: Extended users table with `preferred_services` (TEXT[]), `preferred_providers` (TEXT[]), `saved_addresses` (JSONB), `default_address`, `default_city`, and `default_postal_code` columns. (4) **API Routes**: Added GET `/api/user/profile` (authenticated) and PATCH `/api/user/profile` (authenticated) endpoints with support for updating all preference fields. (5) **Profile Page Integration**: Added two new cards to Profile Preferences tab in `/profile` page showing service selector grid and saved providers list with inline editing and immediate feedback via toast notifications.
