...
- **Electrical Service Enhancements (Nov 14, 2025)**: Updated electrical services booking to match plumbing service structure for UI/UX consistency: (1) **Urgency Selection**: Added required urgency field with 4 levels (Emergency 2.5x, Urgent 1.8x, Standard 1.0x, Scheduled 0.9x discount) using multiplier-based pricing appropriate for electrical work. Info boxes explain pricing impact for emergency/urgent selections and savings for scheduled bookings. (2) **Insurance Checkbox**: Added insurance coverage checkbox matching plumbing pattern for insurer requirements. (3) **3-Button CTA Layout**: Implemented enhanced step 4 footer with three bordered buttons ("Back", "Add to Cart & Select Another", "Add & Go to Cart") providing streamlined cart workflow. (4) **Validation**: Urgency now required field at step 1 alongside electrical issue selection, preventing progression without explicit choice. (5) **Feature Flag**: Added isElectrical flag and included in showEnhancedProviderDetails to enable consistent UI patterns across House Cleaning, Plumbing, and Electrical services.
